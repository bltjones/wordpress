...
class NewRoyalSliderMain {
	...
	function __construct( $file ) {
		...
		add_action( 'wpmu_new_blog', array(&$this, 'new_blog_added') );
	...
	function init() {
		...
		if( !is_admin() ) {
			...
			global $wp_scripts;
			if(isset($wp_scripts)) {
				if ( version_compare($wp_scripts->registered['jquery']->ver, '1.7') == -1 ) {
					wp_deregister_script('jquery');
					wp_register_script('jquery', NEW_ROYALSLIDER_PLUGIN_URL . 'lib/royalslider/jquery-1.7.2.min.js');
				}
			}
